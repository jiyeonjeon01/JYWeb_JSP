...
<section class="normal-board-section">
<article>
    <h2 class="normal-board-title">자유게시판 목록</h2>
    <div class="normal-board-write-link">
        <a href="<%=request.getContextPath()%>/board/normal/write/normalForm.jsp">글쓰기</a>
    </div>
    <%
        if (count == 0) { // 게시글이 없을 경우
    %>
    <table class="normal-board-table">
        <tr>
            <td class="normal-board-empty" colspan="6">게시판에 저장된 글이 없습니다.</td>
        </tr>
    </table>
    <% } else { // 게시글이 있을 경우 %>
    <table class="normal-board-table">
        <tr class="normal-board-header">
            <th>번호</th>
            <th>제목</th>
            <th>작성자</th>
            <th>작성일</th>
    ...
    <br>
    <div class="normal-board-pagination">
        <%
            if (count > 0) { // 페이징 처리
                int pageBlock = 3; // 보여줄 페이지 수
                int pageCount = (int) Math.ceil((double) count / pageSize); // 전체 페이지 수
